...
export function WebView({ url, active, onMeta, registerNav }: Props) {
  ...
  useEffect(() => {
    ...
    (async () => {
      try {
        const controller = await initProxy(currentWisp, url);
        ...
        if (engine === "scramjet" && typeof controller?.["createFrame"] === "function") {
          ...
          // Ultraviolet execution via Service Worker fallback
    ...
    if (engine === "scramjet" && frameRef.current) {
      try {
        ...
        if (iframeRef.current) {
          iframeRef.current.src = getProxyUrl(url, "scramjet");
        }
      }
  ...
  const handleRetryWithNextRelay = () => {
    ...
    setCurrentWisp(available[nextIdx]?.url || "wss://wisp.mercurywork.shop/");
    setRetryCount((c) => c + 1);
  };
  ...
  return (
    <div className="relative h-full w-full bg-background" data-active={active}>
      {/* Primary Webview Host Container */}
      <div ref={hostRef} className={`h-full w-full ${error ? "hidden" : "block"}`} />

      {/* Error & Fallback Overlay Screen */}
      {error && (
        <div className="frosted-grid flex h-full w-full flex-col items-center justify-center p-6 text-center text-foreground bg-background">
          <motion.div
            initial={{ opacity: 0, scale: 0.95, y: 10 }}
            animate={{ opacity: 1, scale: 1, y: 0 }}
            className="w-full max-w-md rounded-xl border border-border bg-neutral-950 p-6 shadow-2xl"
          >
            <div className="mx-auto flex h-12 w-12 items-center justify-center rounded-xl border border-border bg-secondary text-foreground shadow-lg">
              <AlertCircle className="h-6 w-6" />
            </div>

            <h2 className="mt-4 text-lg font-light tracking-tight text-foreground">
              Unable to Load Destination
            </h2>
            <p className="mt-2 text-xs text-muted-foreground">
              Failed to connect to{" "}
              <span className="font-mono text-foreground font-medium">{url}</span>.
            </p>
            ...
            <div className="mt-6 flex flex-col gap-2">
              ...
              <button
                ...
                disabled={retrying}
                className="flex items-center justify-center gap-2 rounded-lg border border-border bg-secondary py-2.5 text-xs font-medium text-secondary-foreground transition-colors hover:bg-neutral-800 disabled:opacity-50"
              >
