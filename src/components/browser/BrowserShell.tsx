...
import { stripProtocol, toUrl } from "@/lib/proxy";
...
export function BrowserShell() {
  ...
  const addTab = (patch: Partial<Tab> = {}) => {
    ...
    if (tab.kind === "new") {
      ...
      setOmnibox(stripProtocol(tab.url ?? ""));
    ...
    setTabs((prev) => {
      ...
      if (id === activeId) {
        ...
        if (targetTab.kind === "new") {
          ...
          setOmnibox(stripProtocol(targetTab.url ?? ""));
          ignoreMetaUrlRef.current = false;
        }
      }
      return next;
  ...
  return (
    <div className="relative flex h-screen flex-col overflow-hidden bg-background text-foreground font-sans select-none">
      {/* 1. Pitch-Black Top Tab Bar */}
      <div className="flex h-11 items-center bg-background px-3 pt-1.5 border-b border-border">
        {/* Tab Strip */}
        <div className="flex flex-1 items-center gap-1 overflow-x-auto no-scrollbar">
              ...
              return (
                <div key={tab.id} className="relative flex items-center">
                  ...
                  >
                    {/* Tab Icon */}
                    {tab.kind === "games" ? (
                      <Gamepad2 className="h-3.5 w-3.5 shrink-0 text-foreground" />
                    ) : tab.kind === "game" ? (
                      tab.icon ? (
                        <img
                          src={tab.icon}
                    ...
                    <span className="flex-1 truncate font-normal text-xs">
                      {tab.kind === "games" ? "Lessons" : tab.title || "New Tab"}
                    </span>
                    ...
                    <button
                      ...
                      className="flex h-4 w-4 items-center justify-center rounded-full text-muted-foreground hover:bg-neutral-800 hover:text-foreground transition-colors"
                    >
                      <X className="h-3 w-3" />
                    </button>
                  </motion.div>
          ...
          <button
            aria-label="New tab"
            onClick={() => addTab()}
            className="flex h-7 w-7 items-center justify-center rounded-lg text-muted-foreground hover:bg-neutral-950 hover:text-foreground transition-colors ml-1"
          >
      ...
      <div className="flex h-12 items-center gap-2.5 bg-background px-3 border-b border-border">
        {/* Navigation buttons */}
        <div className="flex items-center gap-1 shrink-0">
          ...
          <button
            ...
            className="flex h-8 w-8 items-center justify-center rounded-lg text-muted-foreground hover:text-foreground hover:bg-secondary transition-colors"
          >
            <RotateCw className={`h-4 w-4 ${isReloading ? "animate-spin text-foreground" : ""}`} />
          </button>
        </div>
        ...
        >
          <div
            className={`flex h-[34px] items-center gap-2.5 rounded-lg bg-secondary border px-3 text-xs transition-all ${
              isOmniboxFocused
          ...
          >
            ...
            {active.url && (
              ...
              >
                <Star
                  className={`h-4 w-4 ${
                    isCurrentBookmarked ? "fill-white text-foreground" : "text-muted-foreground"
                  }`}
                />
              </button>
            )}
                ...
                onMeta={(meta) => {
                  setTabs((prev) =>
                    prev.map((t) => {
                      ...
                      if (t.kind === "game" || t.kind === "games") return t;
                  ...
                  if (meta.url && tab.id === activeId && !ignoreMetaUrlRef.current) {
                    setOmnibox(stripProtocol(meta.url));
                  }
                }}
                registerNav={(nav) => {
                  navs.current[tab.id] = nav;
